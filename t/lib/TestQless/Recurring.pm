...
sub test_change_tags : Tests {
	my $self = shift;
	$self->time_freeze;
	my $jid = $self->{'q'}->recur('Qless::Job', {'test'=>'test_change_tags'}, interval => 1, tags => ['foo', 'bar']);
	is $self->{'q'}->pop->complete, 'complete';
	$self->time_advance(1);
	is_deeply $self->{'q'}->pop->tags, ['foo', 'bar'];
	# Now let's untag the job
	$self->{'client'}->jobs($jid)->untag('foo');
	is_deeply $self->{'client'}->jobs($jid)->tags, ['bar'];
	$self->time_advance(1);
	is_deeply $self->{'q'}->pop->tags, ['bar'];
	# Now let's add 'foo' back in, and also add 'hey'
	$self->{'client'}->jobs($jid)->tag('foo', 'hey');
	is_deeply $self->{'client'}->jobs($jid)->tags, ['bar', 'foo', 'hey'];
	$self->time_advance(1);
	is_deeply $self->{'q'}->pop->tags, ['bar', 'foo', 'hey'];
	$self->time_unfreeze;
}
...
sub test_peek : Tests {
	my $self = shift;
	$self->time_freeze;
	my $jid = $self->{'q'}->recur('Qless::Job', {'test'=>'test_peek'}, interval => 100);
	is $self->{'q'}->pop->complete, 'complete';
	is $self->{'q'}->peek, undef;
	$self->time_advance(110);
	isnt $self->{'q'}->peek, undef;
	is $self->{'q'}->pop->complete, 'complete';

	# If we are popping fewer jobs than the number of jobs that would have
	# been scheduled, it should only make that many available
	$self->time_advance(800);
	my @jobs = $self->{'q'}->peek(5);
	is scalar @jobs, 5;
	is $self->{'q'}->length, 5;
	foreach my $job ($self->{'q'}->pop(100)) {
		$job->complete;
	}
	is $self->{'q'}->length, 0;
	...
	# Even if there are several recurring jobs, both of which need jobs
	# scheduled, it only pops off the needed number
	$self->{'q'}->recur('Qless::Job', {'test'=>'test_peek'}, interval => 10);
	$self->time_advance(800);
	@jobs = $self->{'q'}->peek(5);
	is scalar @jobs, 5;
	is $self->{'q'}->length, 5;
	...
	# as many recurring jobs as needed
	$self->time_advance(800);
	$self->{'q'}->put('Qless::Job', {'foo'=>'bar'}, priority => 10);
	@jobs = $self->{'q'}->peek(5);
	is scalar @jobs, 5;
	is $self->{'q'}->length, 6;
